...
export async function GET() {
  ...
  try {
    ...
    for (const inst of installations) {
      ...
      const repos = await listInstallationRepos(inst.installationId, session.githubToken);
    ...
    // If the user's GitHub token is stale / revoked, surface a 401 so the
    // frontend can trigger a silent re-auth instead of showing a dead-end error.
